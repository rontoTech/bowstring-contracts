...
/// @notice UUPS-proxied fee configuration and collection with protocol/curator splits.
contract FeeManagerUpgradeable is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    using SafeERC20 for IERC20;

    uint16 public constant MAX_ENTRY_FEE_BPS = 100;
    uint16 public constant MAX_EXIT_FEE_BPS = 200;
    uint16 public constant MAX_MANAGEMENT_FEE_BPS = 500;
    uint16 public constant MAX_PERFORMANCE_FEE_BPS = 3000;
    uint16 public constant MIN_PROTOCOL_SHARE_BPS = 2000;
    uint16 public constant BPS_DENOMINATOR = 10000;

    uint16 public defaultEntryFeeBps;
    ...
    mapping(address => uint256) public accumulatedCuratorFees;
    uint256 public totalAccumulatedCuratorFees;
    ...
    function initialize(address treasury_, address baseAsset_, address owner_) external initializer {
        __Ownable_init(owner_);
        require(treasury_ != address(0), "FeeManager: zero treasury");
        require(baseAsset_ != address(0), "FeeManager: zero base asset");
        protocolTreasury = treasury_;
        baseAsset = IERC20(baseAsset_);
        defaultEntryFeeBps = 30;
        defaultExitFeeBps = 50;
        defaultManagementFeeBps = 50;
        defaultPerformanceFeeBps = 1500;
    }
    ...
    function recordFees(uint256 totalFeeAmount) external {
        ...
        VaultFeeConfig memory config = vaultFees[msg.sender];
        ...
        emit FeesAccumulated(msg.sender, protocolAmount, curatorAmount);
    }
